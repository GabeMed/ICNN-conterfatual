"""
Projects W^(z) to [MIN_WEIGHT, ∞) to maintain convexity while preventing neuron death.

Uses a small positive floor (1e-4) instead of hard zero to:
1. Maintain convexity (W > 0 is strictly positive definite)
2. Allow gradient recovery (prevents exact zeros)
3. Provide numerical stability

This is mathematically valid - strict positivity (W > ε > 0) is stronger than
non-negativity (W ≥ 0) and thus preserves convexity.
"""
function enforcing_convexity!(model::AbstractICNN)
    MIN_WEIGHT = 1f-2  # Changed from 1e-4 to 1e-2 (100x larger) - extreme parameter to prevent neuron death
...
"""
Initializes W^(z) to positive values using improved strategy.

This uses a combination of:
1. Absolute value of initial weights (makes positive)
2. Scaling factor based on layer dimensions (maintains gradient flow)
3. Small positive offset (prevents immediate death)

The scaling follows a modified Xavier initialization adapted for
...
function initialize_convex!(model::AbstractICNN)
    @inbounds for (i, layer) in enumerate(model.hidden_layers)
        ...
        # Extreme parameter to give network a "running start" far from death zone
        layer.weight .= 0.1f0 .+ 0.9f0 .* rand(Float32, size(layer.weight))

        @info "Initialized hidden_layer[$i]: min=$(minimum(layer.weight)), max=$(maximum(layer.weight)), mean=$(mean(layer.weight))"
    end
end
